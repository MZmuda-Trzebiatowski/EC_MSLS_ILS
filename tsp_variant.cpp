...
// ------------------------------------ ASSIGNMENT 6: MSLS, ILS ------------------------------------ //

double msls(const vector<vector<int>> &d, const vector<Node> &nodes) {
    const int ITERATIONS = 200;      // number of local-search runs inside one MSLS run
    const int N_EXPERIMENTS = 20;    // number of independent MSLS runs (to collect statistics)
    
    int n = d.size();
    int k = (n + 1) / 2; // keep same k as main

    vector<int> msls_best_objectives; // best objective of each MSLS run
    msls_best_objectives.reserve(N_EXPERIMENTS);
    ...
    {
        int best_obj_this_msls = INT_MAX;
        vector<int> best_tour_this_msls;

        for (int it = 0; it < ITERATIONS; ++it)
        {
            // start from a random solution of size k
            vector<int> start_tour = random_solution(n, k);

            // apply steepest local search (use 2-opt intra-route)
            vector<int> final_tour = local_search_steepest(start_tour, d, nodes, false);

            int obj = tour_objective(final_tour, d, nodes);

            if (obj < best_obj_this_msls)
            {
                best_obj_this_msls = obj;
                best_tour_this_msls = final_tour;
            }
        }

        // record best objective from this MSLS run
        msls_best_objectives.push_back(best_obj_this_msls);

        // update global best across all MSLS runs
        if (best_obj_this_msls < global_best_obj)
        {
            global_best_obj = best_obj_this_msls;
            global_best_tour = best_tour_this_msls;
        }

        cout << "MSLS run " << (exp + 1) << "/" << N_EXPERIMENTS << " finished. Best obj this run: " << best_obj_this_msls << "\n";
    }
    ...
    print_stats("MSLS (Steepest, 2-opt, Random_INIT)", msls_best_objectives, total_time_ms);

    export_tour_svg("best_msls_steepest_2opt_tour.svg", global_best_tour, nodes);
    export_tour_txt("best_msls_steepest_2opt_tour.txt", global_best_tour);
    cout << "Exported global best MSLS tour (obj=" << global_best_obj << ").\n";

    return total_time_ms / 20.0;
}

// ---------------------------------------------------------------------------
// ITERATED LOCAL SEARCH (ILS)
// perturbation = swap X random nodes in the tour
// X = 20 random nodes  => 10 swaps
// ---------------------------------------------------------------------------

vector<int> perturb(const vector<int>& tour, int X = 20)
{
    mt19937 rng{random_device{}()};
    vector<int> new_tour = tour;
    int m = tour.size();

    int num_swaps = X / 2;  // each swap affects 2 nodes
...
// ---------------------------------------------------------------------------
// ILS main procedure
// Runs until time > time_limit_ms
// Returns: best solution found, writes statistics, counts LS runs
// ---------------------------------------------------------------------------
struct ILSResultStats {
    int X;
    double avg_score;
    int min_score;
    int max_score;
...
void ils(const vector<vector<int>>& d,
         const vector<Node>& nodes,
         double time_limit_ms,
         int X)
{
    const int N_EXPERIMENTS = 20;

    int n = d.size();
    int k = (n + 1) / 2;

    vector<int> best_overall;
    int best_overall_obj = INT_MAX;

    vector<int> ils_results; 
    vector<int> ls_counts;   // number of local searches per ILS run
    ...
    for (int run = 0; run < N_EXPERIMENTS; run++)
    {
        ...
        // 1. Initial solution
        vector<int> current = random_solution(n, k);
        current = local_search_steepest(current, d, nodes, false);

        int current_obj = tour_objective(current, d, nodes);

        vector<int> best = current;
        int best_obj = current_obj;

        int ls_runs = 1;  // we already ran 1 LS

        // 2. ILS Loop
        while (true)
        {
            auto now = chrono::high_resolution_clock::now();
            double elapsed_ms = chrono::duration_cast<chrono::milliseconds>(now - start_run).count();

            if (elapsed_ms >= time_limit_ms)
                break;

            // --- Perturbation ---
            vector<int> perturbed = perturb(best, X);

            // --- Local search ---
            vector<int> improved = local_search_steepest(perturbed, d, nodes, false);
            ls_runs++;

            int obj = tour_objective(improved, d, nodes);
        ...
        // record results
        ils_results.push_back(best_obj);
        ls_counts.push_back(ls_runs);
    ...
    double total_time_ms = chrono::duration_cast<chrono::milliseconds>(global_end - global_start).count();

    print_stats("ILS (Steepest, 2-opt, perturbed with 20-node swap)", ils_results, total_time_ms);

    // Print LS run counts
    cout << "\nNumber of basic LS runs per ILS execution:\n";
    for (int x : ls_counts) cout << x << " ";
    cout << "\n";

    export_tour_svg("best_ils_steepest_2opt_tour.svg", best_overall, nodes);
    export_tour_txt("best_ils_steepest_2opt_tour.txt", best_overall);
}


struct ILSBatchResult {
    int min_score;
    int max_score;
    double avg_score;
};

ILSBatchResult ils_run_batch(const vector<vector<int>>& d,
                             const vector<Node>& nodes,
                             double time_limit_ms,
                             int X,
                             int R = 20)
{
    ...
    for (int r = 0; r < R; r++)
    {
        // ---- run ONE ILS instance ----
        vector<int> current = random_solution(nodes.size(), (nodes.size() + 1) / 2);
        current = local_search_steepest(current, d, nodes, false);
        ...
        {
            auto now = chrono::high_resolution_clock::now();
            double elapsed_ms = chrono::duration_cast<chrono::milliseconds>(now - start).count();
            if (elapsed_ms >= time_limit_ms)
                break;

            vector<int> p = perturb(current, X);
            vector<int> improved = local_search_steepest(p, d, nodes, false);
            int obj = tour_objective(improved, d, nodes);
        ...
        scores.push_back(current_obj);
    }

    ILSBatchResult out;
    out.min_score = *min_element(scores.begin(), scores.end());
    out.max_score = *max_element(scores.begin(), scores.end());
    out.avg_score = accumulate(scores.begin(), scores.end(), 0.0) / scores.size();

    return out;
}

void run_ils_x_experiment(const vector<vector<int>>& d,
                          const vector<Node>& nodes,
                          double time_limit_ms)
{
    vector<int> test_X = {24, 28};
    vector<ILSResultStats> all_stats;

    cout << "\n===== Running ILS Perturbation Strength Experiment =====\n";

    for (int X : test_X)
    {
        cout << "\n--- Testing X = " << X << " ---\n";

        // Run ILS 20 times INTERNALLY
        auto res = ils_run_batch(d, nodes, time_limit_ms, X);

        cout << "min=" << res.min_score
             << ", avg=" << res.avg_score
             << ", max=" << res.max_score << "\n";

        ILSResultStats s;
        s.X = X;
        s.min_score = res.min_score;
        s.max_score = res.max_score;
        s.avg_score = res.avg_score;
...
int main(int argc, char **argv)
{
    ...
    cout << "\n=============== Running MSLS ===============\n";
    double avg_msls_time_ms = msls(d, nodes);

    cout << "\n===== Running ILS =====\n";
    ils(d, nodes, avg_msls_time_ms, 20);

    // Step 1: run MSLS to get time limit
    // cout << "Running MSLS to obtain time limit...\n";
    // double avg_msls_ms = msls(d, nodes);
    // cout << "Average MSLS runtime = " << avg_msls_ms << " ms\n";

    // // Step 2: run the experiment
    // run_ils_x_experiment(d, nodes, avg_msls_ms);
}